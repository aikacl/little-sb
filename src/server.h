...
private:
  ...
  void publish(std::string const &name, std::string_view const message)
  {
    _publishing_name_to_session.at(name)->write(
        Sb_packet{Sb_packet_sender{Sb_packet_sender::Type::Server, "Server"},
                  Sb_packet_type::Message, message});
    spdlog::info("Server->{}: {}", name, message);
  }
               ...
               std::string_view const message)
  {
    session->write(
        Sb_packet{Sb_packet_sender{Sb_packet_sender::Type::Server, "Server"},
                  Sb_packet_type::Message, message});
    spdlog::info("{}->{}: {}", _my_name, to, message);
  }
  ...
  std::unordered_map<std::string, Session_ptr> _publishing_name_to_session;
  std::unordered_map<Session_ptr, std::string> _publishing_session_to_name;
  asio::io_context _io_context;
  tcp::acceptor _acceptor;
  std::error_code _ec;
  std::string_view _my_name{"Server"};
